...
RUN apt-get update \
 && DEBIAN_FRONTEND=noninteractive apt-get install -y acl sudo locales \
      ...
      postgresql-${PG_VERSION}-pg-qualstats postgresql-${PG_VERSION}-hypopg git sysbench \
 ...
 && git clone https://github.com/Percona-Lab/sysbench-tpcc.git /usr/sysbench-tpcc
